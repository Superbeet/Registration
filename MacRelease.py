...
def packageProgram(entry_path, setup_file, icon_file):
    print "[Package Program]%s"%(entry_path)
    cmd = "python %s py2app --iconfile %s " %(setup_file, icon_file)
...
if __name__ == "__main__":
    ...
    mac_dist_dir = os.path.join(pathProgram(),'dist_mac')
...
#     cleanFolder(release_dir)
    ...
    icon_path = os.path.join(pathProgram(), "release", "Program", "seagate_logo.icns")
    
    packageProgram(entry_path, setup_path, icon_path)
    
    renameFolder(dist_dir, mac_dist_dir)
